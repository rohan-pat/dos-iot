...
extern void	irq_set(uint32);
extern void	irq_clear(uint32);
extern void	irq_vec_print(uint32);
...
/* Changes for project */
extern uint32 pin_setdir(int32 , uint32 , uint32);
extern void pin_high(uint32 , uint32 );
extern void pin_low(uint32 , uint32 );
extern uint32 is_high(uint32 , uint32 );
extern uint32 is_low(uint32 port, uint32 pin);
extern uint32 GPIO_set_dir(int32 , uint32 , uint32 );
extern void GPIO_high(uint32 gpio ,uint32 pinNumber);
extern void GPIO_low(uint32 gpio ,uint32 pinNumber);
extern uint32 GPIO_read(uint32 gpio, uint32 pinNumber);

/* led functions. */
extern void ledhandler(uint32	xnum);
extern devcall ledinit(struct dentry *);
extern devcall ledwrite(struct dentry *, char *, uint32);

/* ADC functions. */
//extern devcall adcinit(struct dentry *);
extern devcall adcinit(struct dentry *);
extern devcall adcopen(struct dentry *, char *);
extern devcall adcclose(struct dentry *);
extern devcall adcread(struct dentry	*, char	*, uint32);
extern void adchandler(uint32);
