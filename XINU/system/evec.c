...
// changes for project.
/*-------------------------------------------------------------------------
 * ird_set - trigger interrupt for irq number.
 *-------------------------------------------------------------------------
 */
void	irq_set(uint32 xnum)
{
	int bankNo = 0;
	int irqNo = 0;

	//checking the bank number and the
	bankNo = xnum / 32;
	irqNo = xnum % 32;

	//accessing the IRQ status register.
	struct	intc_csreg *csrptr = (struct intc_csreg *)0x48200000;
	uint32 ls = 1<<irqNo;
	csrptr->banks[bankNo].isr_set = csrptr->banks[bankNo].isr_set | ls;
}

/*-------------------------------------------------------------------------
 * ird_clear - reset interrupt for irq number.
 *-------------------------------------------------------------------------
 */
void	irq_clear(uint32 xnum)
{
	int bankNo = 0;
	int irqNo = 0;
	...
	struct	intc_csreg *csrptr = (struct intc_csreg *)0x48200000;
	uint32 ls = 1<<irqNo;
	csrptr->banks[bankNo].isr_clear = csrptr->banks[bankNo].isr_clear | ls;
}

/*-------------------------------------------------------------------------
 * irq_vec_print - reset interrupt for irq number.
 *-------------------------------------------------------------------------
 */
void	irq_vec_print(uint32 xnum)
{
	kprintf("\nfor irq - %d, handler= %d", xnum, intc_vector[xnum]);
}
